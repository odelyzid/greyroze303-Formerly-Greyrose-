//**********************************************************************************************************************
/**
 *  Scales the image down by 4 for an overall scale of 1/16, modeled after the Microsoft DirectX SDK example
 *  Needs to be its own shader, apart from the 4x DownFilter, so each one can have its own texel table.
 *
...
int BloomDownFilter16Count : GLOBAL = 16;
float2 BloomDownFilter16PixelCoords[16] : GLOBAL =
{
   { 1.5,  -1.5 },
   { 1.5,  -0.5 },
   { 1.5,   0.5 },
...
float2 BloomDownFilter16TexelCoords[16] : GLOBAL;
...
float4 DownFilter( in float2 Tex : TEXCOORD0 ) : COLOR0
{
   ...
   for (int i = 0; i < 16; i++)
   {
      color += tex2D( g_samSrcColor, Tex + BloomDownFilter16TexelCoords[i].xy );
   }
